...
def test_replay_workflow_manifest_reruns_model_selection_and_reports_equivalence(
    tmp_path: Path,
) -> None:
    ...
    report = replay_workflow_manifest(
        ...
        out_dir=tmp_path / "model-selection-replay",
    )

    assert report.workflow == "model-selection"
    ...
    assert report.comparisons[0].label == "selected-model"
    assert report.comparisons[0].status == "equivalent"


def test_replay_workflow_manifest_rejects_changed_inputs(tmp_path: Path) -> None:
    executable = _fake_iqtree(tmp_path / "iqtree-fixture")
...
def test_replay_workflow_manifest_reports_engine_version_drift_for_fasta_to_tree(
    ...
    iqtree = _fake_iqtree(tmp_path / "iqtree-fixture", version="2.9.9")
    drifted_iqtree = _fake_iqtree(tmp_path / "iqtree-drifted", version="3.0.0")
    ...
    report = replay_workflow_manifest(
        ...
        executables={
            "mafft": mafft,
            "trimal": trimal,
            "iqtree": drifted_iqtree,
        },
    )

    assert report.workflow == "fasta-to-tree"
    assert report.engine_version_drift_detected is True
    assert report.outputs_equivalent is True
    assert any(
        drift.expected != drift.observed and drift.label == "iqtree"
