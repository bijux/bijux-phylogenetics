...
def test_fixed_rate_empirical_protein_deep_tree_low_probability_fixture_stays_finite() -> (
    None
):
    tree = _deep_tree()
    ...
    report = evaluate_empirical_protein_tree_likelihood(
        tree,
        records,
    ...
    assert report.log_likelihood < -745.0
    assert math.isclose(report.log_likelihood, expected, rel_tol=0.0, abs_tol=1e-12)


def test_discrete_gamma_empirical_protein_deep_tree_matches_high_precision_mixture() -> (
    None
):
    tree = _deep_tree()
    records = _deep_tree_records()
    ...
    report = evaluate_empirical_protein_tree_likelihood_with_discrete_gamma(
        tree,
        records,
        rate_matrix=rate_matrix,
        root_prior=root_prior,
    ...
    expected = _high_precision_weighted_logsumexp(
        category_log_likelihoods,
