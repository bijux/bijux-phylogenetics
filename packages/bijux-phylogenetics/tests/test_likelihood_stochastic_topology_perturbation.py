...
def test_likelihood_stochastic_topology_perturbation_records_reproducible_nni_path() -> None:
    left_report = search_nucleotide_likelihood_stochastic_topology_perturbation_from_alignment(
    ...
    right_report = search_nucleotide_likelihood_stochastic_topology_perturbation_from_alignment(
        fixture("trees", "jc69_likelihood_nni_start_tree_4_taxa.nwk"),
        fixture("alignments", "jc69_likelihood_nni_alignment_4_taxa.fasta"),
        model_name="jc69",
        perturbation_move_family="nni",
    ...
    assert left_report.algorithm == "nucleotide-likelihood-stochastic-topology-perturbation-search"
    ...
    assert left_report.local_search_start_tree_newick == "((A:0.999999999756461,D:2.43539016857288e-10):2.43539016857288e-10,(B:0.999999999756461,C:2.43539016857288e-10):2.43539016857288e-10);"
    ...
    assert [
        step.tree_after_newick for step in left_report.perturbation_steps
    ] == [
...
def test_likelihood_stochastic_topology_perturbation_records_topology_changing_spr_path(
    ...
    report = search_nucleotide_likelihood_stochastic_topology_perturbation_from_alignment(
    ...
    assert report.perturbed_tree_newick == "((((A:0.05,E:0.1):0.05,B:0.05):0.05,D:0.1):0.1,C:0.1);"
...
def test_write_likelihood_stochastic_topology_perturbation_artifacts_materializes_governed_output_family(
    ...
    assert outputs["perturbation_trace_path"].read_text(encoding="utf-8").startswith(
    ...
    assert payload["algorithm"] == "nucleotide-likelihood-stochastic-topology-perturbation-search"
