...
def test_topology_gateway_exports_rooted_nni_neighbor_surface() -> None:
    ...
    assert (
        topology_api.iter_rooted_nni_move_candidates is iter_rooted_nni_move_candidates
    )
...
def test_rooted_nni_neighbors_preserve_taxa_and_generate_unique_valid_topologies() -> (
    None
):
    report = enumerate_rooted_nni_neighbors(loads_newick("(((A,C),B),D);"))
...
def test_rooted_nni_validation_accepts_binary_root_representation_without_rooted_flag() -> (
    None
):
...
def test_rooted_nni_validation_rejects_nonbinary_rooted_representation() -> None:
    with pytest.raises(
        ValueError, match="rooted NNI enumeration requires a binary root"
    ):
...
def test_write_rooted_nni_artifacts_materializes_governed_outputs(
    tmp_path: Path,
) -> None:
    report = enumerate_rooted_nni_neighbors(fixture("example_tree.nwk"))
