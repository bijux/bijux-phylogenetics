...
from bijux_phylogenetics.phylo.topology.rooted_nni import enumerate_rooted_nni_neighbors
...
def test_spr_topology_proposal_reaches_spr_only_neighbor_without_taxon_loss() -> None:
    ...
    nni_topology_fingerprints = {
        ...
        for row in enumerate_rooted_nni_neighbors(current_tree).neighbor_rows
    }
    ...
    proposal = propose_spr_topology_move(current_state, Random(0))

    assert proposal.is_valid is True
    assert proposal.invalid_reason is None
    assert proposal.changed_fields == (
        "tree.topology:spr:root:clade:A|B|C|D|E/clade:A|B|C|D/taxon:C:D",
    )
    ...
    assert proposed_topology_fingerprint in spr_topology_fingerprints
    assert proposed_topology_fingerprint not in nni_topology_fingerprints


def test_spr_topology_sampler_scores_and_accepts_rejects_moves() -> None:
    initial_state = _build_scored_spr_state(
        update_prior_components=_topology_preference_prior_components
    )

    report = run_metropolis_hastings_sampler(
        initial_state=initial_state,
        propose_state=propose_spr_topology_move,
        update_prior_components=_topology_preference_prior_components,
    ...
    assert (
        ...
        == _preferred_spr_topology_fingerprint()
    )


def test_spr_topology_proposal_rejects_non_bifurcating_tree() -> None:
...
def _build_spr_start_tree() -> PhyloTree:
    tree = PhyloTree.from_newick("((((A:0.1,D:0.1):0.2,B:0.3):0.4,C:0.7):0.8,E:1.5);")
    tree.rooted = True
    return tree


def _preferred_spr_topology_fingerprint() -> str:
    current_state = _build_scored_spr_state(
        update_prior_components=_zero_prior_components
    )
    proposal = propose_spr_topology_move(current_state, Random(0))
...
def _topology_preference_prior_components(
    ...
    return [
        BayesianPriorComponentState(
            ...
            family="discrete",
            log_prior=0.0
            if topology_fingerprint == _preferred_spr_topology_fingerprint()
            else -5.0,
        )
    ]
