...
def test_validate_tree_set_uncertainty_methods_summary_reference_fixtures_governs_uncertainty_counts() -> None:
    ...
    assert (
        observed["multi_topology_methods_summary_keeps_support_and_instability"]
    ...
    assert (
        observed["single_topology_methods_summary_keeps_clean_lane_explicit"]
        .observed["warning_count"]
        == 0
    )
    assert (
        observed["single_topology_methods_summary_keeps_clean_lane_explicit"]
        .observed["topology_cluster_count"]
        == 1
    )
