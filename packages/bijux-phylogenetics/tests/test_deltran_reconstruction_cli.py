...
def test_phylo_parsimony_deltran_cli_writes_governed_artifacts(
    tmp_path: Path,
    capsys,
) -> None:
    out_dir = tmp_path / "deltran-cli"
    ...
    assert payload["metrics"]["algorithm"] == "deltran"
...
def test_phylo_parsimony_deltran_cli_preserves_branch_change_flags(
    tmp_path: Path,
    capsys,
) -> None:
    out_dir = tmp_path / "deltran-branch-map-cli"
    ...
    assert exit_code == 0
    assert payload["status"] == "ok"
    assert "C\tchar01_branch_mapping\tC|D\tC" in (
        out_dir / "branch_changes.tsv"
    ).read_text(encoding="utf-8")
