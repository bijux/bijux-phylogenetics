...
def test_phylo_parsimony_jackknife_cli_writes_governed_seeded_outputs(
    tmp_path: Path,
    capsys,
) -> None:
    left_out_dir = tmp_path / "jackknife-left"
    right_out_dir = tmp_path / "jackknife-right"
    command = [
        "phylo",
        "parsimony",
        "jackknife",
        str(fixture("jackknife_matrix.tsv")),
        "--method",
        "fitch",
        "--replicate-count",
        "12",
    ...
    assert (left_out_dir / "retained_characters.tsv").is_file()
    ...
    assert (left_out_dir / "retained_characters.tsv").read_text(encoding="utf-8") == (
        right_out_dir / "retained_characters.tsv"
