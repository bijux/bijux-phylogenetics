...
def test_package_tree_gateway_exports_quartet_puzzling_surface() -> None:
    ...
    assert trees_api.build_quartet_puzzling_consensus is build_quartet_puzzling_consensus
    assert trees_api.write_quartet_puzzling_artifacts is write_quartet_puzzling_artifacts
...
def test_quartet_puzzling_recovers_expected_tree_from_compatible_quartets(
    ...
    assert all(row.tied_best_split_taxa == [row.best_split_taxa] for row in report.quartet_rows)
    assert len({row.assembled_tree_newick for row in report.assembly_rows}) == 1
    assert {
        tuple(sorted(split))
        for split in informative_unrooted_splits(consensus_tree, set(report.shared_taxa))
    } == {
        ("A", "B"),
        ("D", "E"),
    }
...
def test_write_quartet_puzzling_artifacts_materializes_bundle(tmp_path: Path) -> None:
    ...
    assert outputs["quartet_scores_path"].read_text(encoding="utf-8").startswith(
    ...
    assert outputs["assembly_scores_path"].read_text(encoding="utf-8").startswith(
