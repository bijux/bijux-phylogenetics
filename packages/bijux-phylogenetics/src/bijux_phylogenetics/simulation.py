...
def _normalize_trait_names(trait_names: list[str] | tuple[str, ...]) -> list[str]:
    normalized = [trait.strip() for trait in trait_names if trait.strip()]
    if len(normalized) < 2:
        raise ValueError(
...
def _normalize_root_states(
    ...
    if root_states is None:
        return [0.0] * len(trait_names)
    if len(root_states) != len(trait_names):
        raise ValueError(
            "root_states length must match the number of correlated Brownian traits"
        )
    return [_round_float(float(value)) for value in root_states]
...
def _normalize_square_matrix(
    ...
    *,
    ...
    for row_index, row in enumerate(rows, start=1):
        if len(row) != size:
            raise ValueError(
                f"{label} row {row_index} must contain {size} values, got {len(row)}"
            )
...
def _covariance_matrix_from_correlation_matrix(
    ...
    trait_standard_deviations: list[float] | tuple[float, ...],
) -> list[list[float]]:
    if len(trait_standard_deviations) != len(correlation_matrix):
        raise ValueError(
    ...
    for row_index, row in enumerate(correlation_matrix):
        covariance_matrix.append(
            [
                _round_float(
                    row[column_index]
                    * normalized_standard_deviations[row_index]
                    * normalized_standard_deviations[column_index]
                )
...
def _resolve_correlated_brownian_covariance_matrix(
    *,
    ...
    if (
        evolutionary_covariance_matrix is not None
        and evolutionary_correlation_matrix is not None
    ):
        raise ValueError(
            "provide either evolutionary_covariance_matrix or evolutionary_correlation_matrix, not both"
        )
    size = len(trait_names)
    if evolutionary_covariance_matrix is not None:
        if trait_standard_deviations is not None:
            raise ValueError(
                "trait_standard_deviations can only be used with evolutionary_correlation_matrix"
            )
        return _normalize_square_matrix(
            evolutionary_covariance_matrix,
            size=size,
            label="evolutionary_covariance_matrix",
        )
    if trait_standard_deviations is None:
        raise ValueError(
            "trait_standard_deviations are required when using evolutionary_correlation_matrix"
        )
    correlation_matrix = _normalize_square_matrix(
        evolutionary_correlation_matrix,
        size=size,
        label="evolutionary_correlation_matrix",
    )
    for index, row in enumerate(correlation_matrix):
        if not math.isclose(row[index], 1.0, rel_tol=1e-12, abs_tol=1e-12):
            raise ValueError(
                "evolutionary_correlation_matrix must have unit diagonal entries"
            )
...
def _cholesky_lower(matrix: list[list[float]]) -> list[list[float]]:
    ...
    for row in range(size):
        for column in range(row + 1):
            ...
            value = matrix[row][column] - partial
            if row == column:
                if value <= 0.0:
                    raise ValueError(
...
def _build_correlated_brownian_collection_summary_rows(
    ...
    for taxon, trait_name in dimension_labels:
        values = values_by_dimension[(taxon, trait_name)]
        rows.append(
            ContinuousTraitSimulationSummaryRow(
                row_kind="tip_distribution",
                label=f"{taxon}|{trait_name}",
...
def simulate_correlated_brownian_traits(
    ...
    *,
    ...
    """Simulate one multivariate Brownian tip-trait realization on a fixed tree."""
...
def simulate_correlated_brownian_trait_collection(
    ...
    *,
    ...
    """Simulate one multivariate Brownian replicate collection on a fixed tree."""
    if replicates < 1:
        raise ValueError(f"replicates must be at least 1, got {replicates}")
    normalized_trait_names = _normalize_trait_names(trait_names)
    normalized_root_states = _normalize_root_states(
...
def write_correlated_continuous_trait_table(
    ...
    """Write one multivariate Brownian tip-trait table."""
    return write_taxon_rows(
        path,
...
def write_correlated_continuous_trait_collection_table(
    ...
    """Write one multivariate Brownian collection ledger across replicates."""
    return write_taxon_rows(
        path,
...
def write_correlated_continuous_trait_collection_summary_table(
    ...
    """Write multivariate Brownian parameter and empirical summary rows."""
    return write_taxon_rows(
        path,
        columns=[
            "row_kind",
