...
_DATASET_ID = "central_european_seashore_flora"
_DATASET_LABEL = "Central European seashore flora dataset"
_TAXON_COLUMN = "species"
_CONTINUOUS_TRAIT = "seed_mass"
_PGLS_PREDICTOR = "plant_height"
_DISCRETE_TRAIT = "lifeform"
_CLADE_TRAIT = "lifeform"
...
def write_central_european_seashore_flora_workflow_bundle(
    output_root: Path, report: CentralEuropeanSeashoreFloraWorkflowReport
...
def _write_workflow_summary_table(
    path: Path, report: CentralEuropeanSeashoreFloraWorkflowReport
) -> Path:
    slope = next(
        coefficient
...
def _write_demo_overview(
    path: Path, bundle: CentralEuropeanSeashoreFloraWorkflowBundle
) -> Path:
    lines = [
