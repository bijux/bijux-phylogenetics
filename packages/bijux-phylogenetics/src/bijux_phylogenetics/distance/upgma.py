...
class UPGMAMergeRow:
    """One deterministic merge step during UPGMA clustering."""
...
@dataclass(frozen=True, slots=True)
class UPGMAClusterHeightRow:
    """One internal-cluster height recovered during UPGMA clustering."""
...
def _choose_merge_pair(
    ...
    if best_pair is None:
        raise InvalidDistanceMatrixError(
            "distance matrix did not yield a valid UPGMA merge pair"
        )
    return best_pair
...
def build_upgma_tree(
    ...
    while len(active_clusters) > 1:
        ...
        left_key, right_key = _choose_merge_pair(active_distances, active_keys)
    ...
    return PhyloTree(root=root, source_format="newick", rooted=True), UPGMABuildReport(
