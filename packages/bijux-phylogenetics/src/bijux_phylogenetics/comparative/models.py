...
def validate_comparative_reference_examples() -> ComparativeReferenceValidationReport:
    """Validate comparative-model outputs against durable internal reference examples."""
    root = Path(__file__).resolve().parents[3] / "tests/fixtures"
    examples = [
        {
            "case": "brownian-example-tree",
            "model": "brownian",
            "tree": root / "trees/example_tree.nwk",
            "traits": root / "metadata/example_traits_comparative.tsv",
            "trait": "response",
            "expected": {
                "root_state": 2.8055555543209874,
                "rate": 4.774305191647407,
            },
        },
        {
            "case": "ou-example-tree",
            "model": "ou",
            "tree": root / "trees/example_tree.nwk",
            "traits": root / "metadata/example_traits_comparative.tsv",
            "trait": "response",
            "expected": {
                "theta": 2.7503175510416416,
                "alpha": 33.333333,
            },
        },
    ]
    ...
    for example in examples:
        ...
        tolerance = 1e-6 if example["model"] == "brownian" else 5e-2
        passed = all(
            math.isclose(observed[name], expected_value, rel_tol=tolerance, abs_tol=tolerance)
            for name, expected_value in example["expected"].items()
        )
        observations.append(
            ComparativeReferenceObservation(
                ...
                model=example["model"],
                trait=example["trait"],
                expected_parameters=example["expected"],
