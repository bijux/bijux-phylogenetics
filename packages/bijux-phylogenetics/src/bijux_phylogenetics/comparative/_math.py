...
def invert_matrix(matrix: list[list[float]]) -> list[list[float]]:
    ...
    augmented = [
        ...
        for row, identity_row in zip(matrix_copy(matrix), identity_matrix(size), strict=True)
    ]
    for pivot_index in range(size):
        ...
        for row_index in range(size):
            ...
            augmented[row_index] = [
                ...
                for row_value, pivot_value in zip(augmented[row_index], augmented[pivot_index], strict=True)
            ]
...
def stable_covariance(matrix: list[list[float]], *, epsilon: float = 1e-8) -> list[list[float]]:
