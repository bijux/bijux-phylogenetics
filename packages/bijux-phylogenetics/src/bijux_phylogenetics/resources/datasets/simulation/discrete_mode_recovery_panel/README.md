...
- `simulation-cases.tsv`: deterministic case definitions, including state vocabularies, explicit transition-rate matrices, seeds, tolerated recovery error, expected model-choice outcomes, and overparameterized-review expectations.
- `expected/`: governed reviewer-facing outputs regenerated from the packaged trees and case table, including Bijux-versus-`geiger` rate comparisons and the stored `geiger` summary ledger used by the benchmark.
